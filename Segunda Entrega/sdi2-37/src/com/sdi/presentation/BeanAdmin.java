...
public class BeanAdmin {
	...
	public void cambiarEstado(User user) {		
		...
		try {
			...
			Log.debug("No se ha podido actualizar el estado del usuario "+user.getLogin().toUpperCase());
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error: ", "No se ha podido actualizar el estado del usuario "+user.getLogin().toUpperCase()));
		}		
	}
	...
	public void eliminarUsuario(User user) {
		...
		try {
			...
			Log.debug("Se ha eliminado el usuario "+user.getLogin().toUpperCase());
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Exito: ", "Se ha eliminado el usuario "+user.getLogin().toUpperCase()));
		} catch(BusinessException e) {
			Log.debug("No se ha podido eliminar el usuario "+user.getLogin().toUpperCase());
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error: ", "No se ha podido eliminar el usuario "+user.getLogin().toUpperCase()));
		}		
	}
}